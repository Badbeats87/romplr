  ...
  while (working) {
    ...
    while (SDL_PollEvent(&sdl_event)) {
      if (sdl_event.type == SDL_KEYDOWN) {
        ...
        // Bank select: 1-4 keys = CC70 val 0-3
        // (0=Internal, 1=Card, 2=Preset A, 3=Preset B)
        // Uses JV-880 bank select: CC0 MSB=80 for Int/Card, 81 for Preset
        auto scancode = sdl_event.key.keysym.scancode;
        if (scancode >= SDL_SCANCODE_1 && scancode <= SDL_SCANCODE_4) {
          uint8_t bank = scancode - SDL_SCANCODE_1;
          uint8_t msb = (bank < 2) ? 80 : 81;
          uint8_t pc  = (bank == 1 || bank == 3) ? 64 : 0;
          uint8_t bankMsg[] = { 0xB0, 0x00, msb };
          mcu.postMidiSC55(bankMsg, 3);
          uint8_t pgmMsg[] = { 0xC0, pc };
          mcu.postMidiSC55(pgmMsg, 2);
          printf("Bank select: %d (CC0 MSB=%d, PC=%d)\n", bank, msb, pc);
        }
        // Program change: up/down arrows
        if (scancode == SDL_SCANCODE_UP || scancode == SDL_SCANCODE_DOWN) {
          static uint8_t currentPatch = 0;
          if (scancode == SDL_SCANCODE_UP && currentPatch < 127)
            currentPatch++;
          if (scancode == SDL_SCANCODE_DOWN && currentPatch > 0)
            currentPatch--;
          uint8_t pgmMsg[] = { 0xC0, currentPatch };
          mcu.postMidiSC55(pgmMsg, 2);
          printf("Program change: %d\n", currentPatch);
        }
        // Note on/off: space bar for quick test
